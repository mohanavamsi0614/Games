...
function Home() {
    ...
    const fetchAll = async () => {
      try {
        const [ideaRes, newsRes] = await Promise.all([
          axios.get("http://localhost:5500/ideas"),
          axios.get("http://localhost:5500/news"),
        ]);
        if (mounted) {
  ...
  const vote = async (ideaId) => {
    ...
    try {
      const res = await axios.post("http://localhost:5500/vote", {
        ideaId,
        userId: userToken,
      });
  ...
  const submitIdea = async () => {
    ...
    try {
      const res = await axios.post("http://localhost:5500/idea", {
        idea: form,
        userId: user._id,
      });
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#f6f9ff] via-[#edf2fb] to-[#e3f2fd] p-6 text-gray-800">
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <div className="flex gap-3">
            <button
              ...
              className="bg-[#E16254] hover:bg-[#d95647] text-white px-4 py-2 rounded-lg font-medium shadow-md hover:scale-105 transition"
            >
              Subscribe
            </button>
            <button
        ...
        <div className="grid grid-cols-3 gap-6">
          {/* Left: Ideas */}
          <motion.div
            className="col-span-2 bg-white rounded-3xl shadow-lg p-6 overflow-y-auto h-[78vh]"
          ...
          {/* Right: News */}
          <motion.div
            className="col-span-1 bg-[#1e3a8a] text-white rounded-3xl p-6 shadow-lg h-[78vh] overflow-y-auto"
            initial={{ opacity: 0, x: 30 }}
            animate={{ opacity: 1, x: 0 }}
          >
            ...
            {news.length === 0 ? (
              ...
              news.slice(0, 8).map((n) => (
                <motion.div
                  key={n._id || n.id}
                  className="bg-white/10 flex justify-around  p-4 w-full rounded-xl mb-3 hover:bg-white/20 transition"
                  whileHover={{ scale: 1.03 }}
                >
                  <img src={n.img} className=" rounded-2xl w-1/2"/>
                  <div>
